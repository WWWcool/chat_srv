...
websocket_handle({text, Message}, #state{state = Cl_state} = State) ->
    Tuple = json_proto:encode(Message),
    ...
    {reply, {text, json_proto:decode(Reply)}, NewState};
...
websocket_info({'DOWN', _Ref, process, _Pid, Reason}, State) ->
    ...
    {reply, {text, json_proto:decode({service_message, "Server down"})}, State};
websocket_info({resend_message, Message}, State) ->
    {reply, {text, json_proto:decode(Message)}, State};
