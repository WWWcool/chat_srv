...
init(_) ->
    ...
    ClientChild = #{id     => chat_client,
                 start  => {chat_client, start_link, [Name]},
                 restart    => transient,
                 shutdown   => 2000,
                 type   => worker,
                 modules    => [chat_client]},
...
%start_client(Num) ->
%    logger:alert("start client "),
%    supervisor:start_child(?MODULE, [Num]).
%
%empty_clients(Max) ->
%    [{ok, _Pid}] = [start_client(Num) || Num <- lists:seq(1,Max)],
%    ok.
